...
class AwsAutomationApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AWS自动化控制面板 V8.0 (集成Outlook API)") # 版本号更新
        ...
        self.gui_settings_file = "gui_settings.json" # 虽然此版本主要逻辑在发牌器，但保留此文件用于未来扩展
        ...
        self.email_api_provider_combo.set('Domain') # 默认选项
    ...
    def fetch_phone_number_from_api(self, numeric_id, phone_number_dialing_code):
        api_url = f"https://api.small5.co/hub/en/proxy.php?action=getNumber&service=am&country={numeric_id}&platform=smshub"
        try:
            ...
            if raw_data.startswith('ACCESS_NUMBER:'):
                ...
                if len(parts) >= 3:
                    ...
                    phone_number_url = f"https://api.small5.co/hub/en/proxy.php?action=getStatus&id={phone_number_id}&platform=smshub"
                    return {
        ...
        while not self.dealer_thread_stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    self.log(f"[GUI-发牌师] 为工作线 {workline_id} 获取地址信息...", instance_id=workline_id)
                    ...
                    self.log(f"[GUI-发牌师] 为工作线 {workline_id} 获取手机号...", instance_id=workline_id)
