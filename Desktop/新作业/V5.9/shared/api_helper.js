...
async function fetchNewPhoneNumber(countryCode) {
    ...
    const apiUrl = `https://api.small5.co/hub/en/proxy.php?action=getNumber&service=am&country=${numericId}&platform=smshub`;
    ...
    try {
        ...
        if (typeof response.data === 'string' && response.data.startsWith('ACCESS_NUMBER:')) {
            ...
            if (parts.length >= 3) {
                ...
                // 根据ID自动组装检查短信的URL
                const phone_number_url = `https://api.small5.co/hub/en/proxy.php?action=getStatus&id=${phone_number_id}&platform=smshub`;
